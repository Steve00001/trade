...
function callGroq(messages, systemPrompt) {
  return new Promise((resolve, reject) => {
    ...
    if (!groqKey) return reject(new Error('GROQ_API_KEY non configuree sur Vercel'));
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (!text) return reject(new Error('Pas de reponse de Groq'));
          resolve(text);
        } catch(e) { reject(e); }
      });
    });
...
- Si l'utilisateur mentionne un trade sans SL rappelle l'importance du SL
